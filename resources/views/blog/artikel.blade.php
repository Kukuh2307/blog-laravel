...
@section('content')
    <!-- Blog entries-->
    <div class="col-lg-8">
        {{-- apabila hasil pencarian ada, maka akan di tampilkan --}}
        @if ($articles->count())
        {{-- jika kata kunci terisi maka akan di tampilkan sebagai label --}}
        @if ($cari)
            <div class="h3 mb-3">{{ $hasil }} - <span class="text-primary">{{ $cari }}</span></div>
        @endif
            <!-- Featured blog post-->
        ...
        @else
            <h3 class="my-2"><i class="fa-solid fa-magnifying-glass"></i> Artikel "<i><span class="text-primary">{{ $cari }}</span></i>" tidak di temukan</h3>
        @endif
        <!-- Pagination-->
